...
describe('Form test', () => {
    ...
    it('should successfully get an empty list of Todos', async () => {
    ...
    it('should successfully add a Todos', async () => {
        const response = await server.inject({ 
            method: 'POST', 
            url: '/api/todos',
            payload: {
                name: 'eat lunch',
                description: 'Eat lunch with Jerry.',
                status: false
            }
        })
        ...
        expect(res.todo.name).toBe('eat lunch')
        expect(res.todo.description).toBe('Eat lunch with Jerry.')
